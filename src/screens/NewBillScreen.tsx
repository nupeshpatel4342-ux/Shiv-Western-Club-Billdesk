...
import { Item, Bill, Settings, CatalogProduct } from "../types";
...
export const NewBillScreen = ({ 
  ...
  const [suggestedCustomer, setSuggestedCustomer] = useState<{ name: string, phone: string, address?: string } | null>(null);
  ...
  const handlePhoneChange = (phoneVal: string) => {
    setCustomer(prev => {
      const updated = { ...prev, phone: phoneVal };
      const normalizedInput = phoneVal.trim().replace(/\D/g, "");
      
      if (normalizedInput.length >= 10) {
        // Find matching customer from past bills
        const match = bills.find(b => {
          const matchPhone = b.customerObj.phone.replace(/\D/g, "");
          return matchPhone.endsWith(normalizedInput) || normalizedInput.endsWith(matchPhone);
        });
        
        if (match) {
          setSuggestedCustomer(match.customerObj);
        } else {
  ...
  return (
    <div className="fade" style={{ padding: isDesktop ? "0 0 100px" : "20px 18px 100px" }}>
      ...
      <div style={{ display: "flex", flexDirection: isDesktop ? "row" : "column", gap: 24, alignItems: "flex-start" }}>
        ...
        {/* Left Column (Customer details + Items list) */}
        <div style={{ flex: 2, width: "100%", display: "flex", flexDirection: "column", gap: 24 }}>
          
          {/* Customer Info */}
          <div style={{ background: C.card, borderRadius: 20, padding: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.03)", border: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              ...
              <div style={{ borderBottom: `1.5px solid ${C.border}`, padding: "8px 0" }}>
                <input value={customer.phone} onChange={e => handlePhoneChange(e.target.value)} placeholder="Mobile Number *" style={{ fontSize: 15, fontWeight: 600, color: C.dark, width: "100%" }} />
              </div>
            </div>
            <div style={{ borderBottom: `1.5px solid ${C.border}`, padding: "8px 0" }}>
            ...
            {suggestedCustomer && (
              <div style={{ marginTop: 14 }}>
                ...
                >
                  👤 Autofill: {suggestedCustomer.name}?
                </button>
              </div>
            )}
          </div>
        ...
        {/* Right Column (Payment summary and action button) */}
        <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", gap: 20, position: isDesktop ? "sticky" : "static", top: isDesktop ? "40px" : "auto" }}>
          
          {/* Payment Type & Summary */}
          <div style={{ background: C.card, borderRadius: 24, padding: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.03)", border: `1px solid ${C.border}` }}>
            {/* Payment Type Toggle */}
            <div style={{ display: "flex", background: C.bg, borderRadius: 14, padding: 4, marginBottom: 24 }}>
              <button type="button" onClick={() => setIsCredit(false)} style={{ flex: 1, padding: "12px", borderRadius: 12, fontSize: 12, fontWeight: 800, background: !isCredit ? C.dark : "transparent", color: !isCredit ? C.accent : C.muted, transition: "0.2s", border: "none", textTransform: "uppercase", letterSpacing: "0.5px", cursor: "pointer" }}>Full Paid</button>
              <button type="button" onClick={() => setIsCredit(true)} style={{ flex: 1, padding: "12px", borderRadius: 12, fontSize: 12, fontWeight: 800, background: isCredit ? C.accent : "transparent", color: isCredit ? C.dark : C.muted, transition: "0.2s", border: "none", textTransform: "uppercase", letterSpacing: "0.5px", cursor: "pointer" }}>Credit (Udhar)</button>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <div style={{ borderBottom: `1.5px solid ${C.border}`, width: 100, textAlign: "right" }}>
                <input type="number" value={discount} onChange={e => setDiscount(Number(e.target.value))} style={{ textAlign: "right", fontSize: 16, fontWeight: 800, color: C.accent, width: "100%", border: "none", outline: "none", background: "transparent" }} placeholder="0" />
              </div>
            </div>
            ...
            {isCredit && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, padding: "12px 16px", background: C.bg, borderRadius: 14 }}>
                ...
                <div style={{ borderBottom: `2px solid ${C.green}`, width: 100, textAlign: "right" }}>
                  <input type="number" value={paidAmount} onChange={e => setPaidAmount(e.target.value === "" ? "" : Number(e.target.value))} style={{ textAlign: "right", fontSize: 16, fontWeight: 800, color: C.green, width: "100%", border: "none", outline: "none", background: "transparent" }} placeholder="0" />
                </div>
              </div>
            )}
      ...
      <AnimatePresence>
        {showAdd && <AddItemModal onAdd={it => setItems([...items, it])} onClose={() => setShowAdd(false)} products={products} />}
